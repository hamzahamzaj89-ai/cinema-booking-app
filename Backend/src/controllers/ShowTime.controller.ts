...
export const filterShowTime: Controller = async (req, res, next) => {
  try {
    if (!req.query.branch) {
    ...
    if (req.query.period && !req.query.date) {
      throw new AppError("Date is required when filtering by period.", 400);
    }

    const match: any = {
      branch: new mongoose.Types.ObjectId(req.query.branch as string),
      isActive: true,
    };
...
export const showTimeSeats: Controller = async (req, res, next) => {
  try {
...
export const getShowtimePrices: Controller = async (req: any, res: any) => {
  try {
    ...
    const prices = await showTimeModel.findById(showTime).select("price -_id");
